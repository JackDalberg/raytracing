...
pub fn init(opts: Options) Camera {
    ...
    const image_height: u16 = @max(1, @as(u16, @intFromFloat(@as(f32, @floatFromInt(image_width)) / aspect_ratio)));
    const max_bounce_depth = opts.max_bounce_depth;

    const pixel_samples_scale: f32 = 1.0 / @as(f32, @floatFromInt(opts.samples_per_pixel));
...
fn getRay(self: Camera, i: usize, j: usize) Ray {
    ...
    const pixel_sample = self.pixel00 + vec.scale(self.pixel_delta_u, offset[0] + @as(f32, @floatFromInt(i))) + vec.scale(self.pixel_delta_v, offset[1] + @as(f32, @floatFromInt(j)));
    ...
    const time = self.rand.float(f32);
...
fn rayColor(self: Camera, ray: Ray, world: Hittable, depth: u16) Color {
    ...
    const rec = world.hit(ray, 0.001, std.math.inf(f32));
    if (rec.is_hit) {
